...
{
    SubtitleController::SubtitleController(std::shared_ptr<Api::ApiClient>api,std::shared_ptr<IPlaybackEngine>engine,std::shared_ptr<Services::SettingsSyncService>settings):m_api(std::move(api)),m_engine(std::move(engine)),m_settings(std::move(settings)){if(!m_api||!m_engine||!m_settings)throw std::invalid_argument("SubtitleController requires dependencies.");}
    ...
    {
        auto const version=++m_generation;auto const ui=winrt::apartment_context{};m_request=request;m_choices.clear();m_display.clear();m_appliedSerial=0;
    ...
    void SubtitleController::Stop()noexcept{++m_generation;if(m_engineToken){m_engine->RemoveChangedHandler(m_engineToken);m_engineToken=0;}m_changed={};m_choices.clear();m_display.clear();m_request=nullptr;}
    ...
    void SubtitleController::OnEngineChanged(){auto const state=m_engine->State();if(!m_request||state.FileSerial==0||state.FileSerial==m_appliedSerial)return;m_appliedSerial=state.FileSerial;ApplyRememberedOrDefault();}
