...
{
    std::string EncodeMpvUtf8(std::wstring const& value)
    {
    ...
    void RunMpvCommand(mpv_handle*handle,std::vector<std::string> const&arguments)
    {
        ...
        CheckMpvResult("mpv command",mpv_command(handle,values.data()));
    }

    void LoadMpvSource(mpv_handle*handle,PlaybackSource const&source)
    {
        if(!handle)throw std::invalid_argument("An initialized mpv handle is required.");
        auto sourceUtf8=EncodeMpvUtf8(source.Location);
        auto headerFields=EncodeMpvUtf8(SerializePlaybackHeaders(source.Headers));
        if(headerFields.empty())
        {
            RunMpvCommand(handle,{"loadfile",std::move(sourceUtf8),"replace"});
            return;
        }
        ...
        CheckMpvResult("mpv loadfile",code);
    }
}
