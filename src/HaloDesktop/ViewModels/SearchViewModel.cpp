...
namespace winrt::HaloDesktop::implementation
{
    ...
    void SearchViewModel::Query(winrt::hstring const&value){if(m_query==value)return;m_query=value;Raise(L"Query");m_debounceTimer.Stop();++m_searchVersion;if(m_query.size()>=2)m_debounceTimer.Start();else{m_loading=false;m_error=false;m_results.Clear();m_topMatch=nullptr;m_movieCount=m_seriesCount=0;RaiseState();}}
    ...
    {auto lifetime=get_strong();auto const uiContext=winrt::apartment_context{};auto const version=++m_searchVersion;if(m_query.size()<2){m_loading=false;m_error=false;m_results.Clear();m_topMatch=nullptr;m_movieCount=m_seriesCount=0;RaiseState();co_return;}m_loading=true;m_error=false;RaiseState();bool failed{};try{co_await m_catalog->SearchAsync(m_query);}catch(...){failed=true;}co_await uiContext;if(version!=m_searchVersion)co_return;m_loading=false;m_error=failed;if(!failed){if(deliberate)m_catalog->RecordRecent(m_query);LoadRecents();Rebuild();}RaiseState();}
