...
namespace winrt::HaloDesktop::implementation
{
    ...
    winrt::hstring DetailViewModel::HeaderTitle()const{return ContentVisibility()==Visible?winrt::hstring{}:Title();}
    winrt::hstring DetailViewModel::Kicker()const{return m_detail?m_detail.Kicker():L"";}winrt::hstring DetailViewModel::MetaLine()const{return m_detail?m_detail.MetaLine():L"";}winrt::hstring DetailViewModel::Synopsis()const{return m_detail?m_detail.Synopsis():L"";}winrt::hstring DetailViewModel::Poster()const{return m_detail&&!m_detail.Poster().empty()?m_detail.Poster():(m_params?m_params.Poster():L"");}winrt::hstring DetailViewModel::Background()const{return m_detail&&!m_detail.Background().empty()?m_detail.Background():Poster();}winrt::hstring DetailViewModel::PrimaryActionLabel()const{return m_primaryActionLabel;}winrt::hstring DetailViewModel::SourceActionLabel()const{return m_params&&m_params.Type()==L"movie"?L"Choose a source":L"Choose episode source";}winrt::hstring DetailViewModel::SeasonMeta()const{return m_params&&m_params.Type()==L"series"?winrt::to_hstring(m_episodes.Size())+L" EPISODES":L"";}winrt::hstring DetailViewModel::LibraryLabel()const{return m_inLibrary?L"In library":L"Add to library";}std::int32_t DetailViewModel::SeasonIndex()const noexcept{return m_seasonIndex;}
    ...
    // A movie shows no episode list, so the row under the hero is otherwise empty:
    // the aside takes the whole width and lays its two cards out side by side from
    // the left edge, rather than sitting alone in a narrow right-hand column. A
    // series keeps that column, because there the episode list is what it sits
    // beside. Both read the episode section's own predicate so they cannot drift
    // apart from what is actually on screen.
    auto DetailViewModel::AsideOrientation()const noexcept->Microsoft::UI::Xaml::Controls::Orientation{return EpisodeSectionVisibility()==Visible?Microsoft::UI::Xaml::Controls::Orientation::Vertical:Microsoft::UI::Xaml::Controls::Orientation::Horizontal;}
    std::int32_t DetailViewModel::AsideColumn()const noexcept{return EpisodeSectionVisibility()==Visible?2:0;}
    std::int32_t DetailViewModel::AsideColumnSpan()const noexcept{return EpisodeSectionVisibility()==Visible?1:3;}
    double DetailViewModel::AsideCardWidth()const noexcept{return EpisodeSectionVisibility()==Visible?292.0:420.0;}
    ...
    void DetailViewModel::BrowseSources(){if(!m_params)return;if(m_params.Type()==L"movie")m_navigation->ShowSheet(::HaloDesktop::Services::Page::Sources,winrt::make<implementation::SourcesNavParams>(m_params.Type(),m_params.MetaId(),m_params.MetaId(),L"movie:"+m_params.MetaId(),Title(),Title(),L"",Poster()));else if(m_episodes.Size())OpenSources(m_episodes.GetAt(0));}
    void DetailViewModel::OpenDownloads(){m_navigation->GoTo(::HaloDesktop::Services::Page::Downloads);}
    // Detail is only ever reached from another shell page, so there is always
    // something behind it; the guard is for a cold navigation in testing.
    void DetailViewModel::GoBack(){if(m_navigation->CanGoBack())m_navigation->GoBack();else m_navigation->GoTo(::HaloDesktop::Services::Page::Home);}winrt::event_token DetailViewModel::PropertyChanged(Microsoft::UI::Xaml::Data::PropertyChangedEventHandler const&h){return m_propertyChanged.add(h);}void DetailViewModel::PropertyChanged(winrt::event_token const&t)noexcept{m_propertyChanged.remove(t);}
    ...
    winrt::Windows::Foundation::IAsyncAction DetailViewModel::ToggleLibraryAsync(){auto lifetime=get_strong();if(!m_params)co_return;auto const ui=winrt::apartment_context{};auto const next=!m_inLibrary;try{auto const applied=co_await m_library->SetMembershipAsync(m_params.Type(),m_params.MetaId(),Title(),m_params.Poster().empty()?std::nullopt:std::optional<winrt::hstring>{m_params.Poster()},next);co_await ui;if(!applied)co_return;m_inLibrary=next;m_catalog->RebuildLibrary();Raise(L"LibraryLabel");}catch(...){}}
    ...
    void DetailViewModel::RaiseState(){for(auto const n:{L"Title",L"HeaderTitle",L"Kicker",L"MetaLine",L"Synopsis",L"Poster",L"Background",L"PrimaryActionLabel",L"SourceActionLabel",L"Episodes",L"Facts",L"Availability",L"Seasons",L"SeasonIndex",L"SeasonMeta",L"LibraryLabel",L"ContentVisibility",L"LoadingVisibility",L"ErrorVisibility",L"EpisodeSectionVisibility",L"AsideOrientation",L"AsideColumn",L"AsideColumnSpan",L"AsideCardWidth"})Raise(n);}void DetailViewModel::Raise(wchar_t const*n){::HaloDesktop::detail::RaisePropertyChanged(m_propertyChanged,*this,n);}
}
